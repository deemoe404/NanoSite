...
function buildSiteUI(root, state) {
  ...
  const createTriStateCheckbox = (section, config) => {
    ...
    const resolveDefaultValue = () => {
      if (!Object.prototype.hasOwnProperty.call(config, 'defaultValue')) return undefined;
      return (typeof config.defaultValue === 'function') ? config.defaultValue() : config.defaultValue;
    };

    const sync = () => {
      applyValue(config.get());
    };
    ...
    resetBtn.addEventListener('click', () => {
      ...
      if (defaultValue === undefined) config.set(null);
      else config.set(defaultValue);
