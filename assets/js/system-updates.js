...
function handleFileToggleClick() {
  if (!currentSummary.length || currentSummary.length <= FILE_LIST_COLLAPSE_THRESHOLD) return;
...
function applySummary(entries, files) {
  ...
  fileListCollapsed = currentSummary.length > FILE_LIST_COLLAPSE_THRESHOLD;
  renderFileList();
  notify();
}
...
function renderFileList() {
  ...
  const toggle = elements.fileToggle;
  ...
  const hasOverflow = currentSummary.length > FILE_LIST_COLLAPSE_THRESHOLD;
  const limit = hasOverflow && fileListCollapsed ? FILE_LIST_COLLAPSE_THRESHOLD : currentSummary.length;
  for (let i = 0; i < limit; i += 1) {
    const entry = currentSummary[i];
  ...
  if (toggle) {
    if (hasOverflow) {
      const hiddenCount = currentSummary.length - FILE_LIST_COLLAPSE_THRESHOLD;
      toggle.hidden = false;
      toggle.removeAttribute('aria-hidden');
      toggle.setAttribute('aria-expanded', fileListCollapsed ? 'false' : 'true');
      toggle.textContent = fileListCollapsed
        ? t('editor.systemUpdates.fileToggle.expand', { hidden: hiddenCount })
        : t('editor.systemUpdates.fileToggle.collapse');
    } else {
      toggle.hidden = true;
      toggle.setAttribute('aria-hidden', 'true');
...
export function initSystemUpdates(options = {}) {
  ...
  elements.fileToggle = document.getElementById('systemUpdateFileToggle');
  ...
  if (elements.fileToggle) {
    elements.fileToggle.addEventListener('click', handleFileToggleClick);
  }
