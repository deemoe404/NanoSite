...
如果文章位於 `wwwroot/post/page/githubpages_zh.md`，圖片應放在 `wwwroot/post/page/page.jpeg`。影片檔案也使用相同規則，NanoSite 會自動辨識並渲染。
...
```markdown
這是一張 [為 NanoSite 設定 GitHub Pages](?id=post%2Fpage%2Fgithubpages_zh.md "card") 行內卡片。
```
...
- `?lang=zh-tw` — UI 語言偏好，會儲存在 localStorage；內容會先嘗試匹配該語言，再按回退鏈選擇可用版本。
