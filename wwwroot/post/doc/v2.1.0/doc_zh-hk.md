...
## 檔案概覽
開始使用 NanoSite 前，可以先認識幾個核心檔案同資料夾：
...
> 你可以由 [v2.1.0/site.yaml](https://github.com/deemoe404/NanoSite/blob/v2.1.0/site.yaml) 取得 v2.1.0 嘅預設設定。
...
## 文章寫作
NanoSite 預設由 `wwwroot/index.yaml` 讀取文章列表。簡化格式如下：
...
tags:
  ...
  - GitHub Pages
image: page.jpeg
excerpt: 你可以將 NanoSite 免費託管喺 GitHub Pages 上。
author: deemoe
ai: true
---
```
...
About:
  ...
  zh-tw:
    title: 關於微站
    location: tab/about/zh-tw.md
  zh-hk:
    title: 關於微站
    location: tab/about/zh-hk.md
  ja:
    title: 概要
...
如果文章位於 `wwwroot/post/page/githubpages_zh.md`，圖片應該放喺 `wwwroot/post/page/page.jpeg`。影片檔案都使用同一套規則，NanoSite 會自動辨識同渲染。

## 站內連結卡片
當一個段落只包含指向文章嘅連結時，NanoSite 會將佢升級成有封面、摘要、日期同閱讀時間嘅卡片：

```markdown
[為 NanoSite 設定 GitHub Pages](?id=post%2Fpage%2Fgithubpages_zh.md)
```

若要喺行內強制使用卡片，可以喺連結 title 入面加入 `card`：

```markdown
呢度係一張 [為 NanoSite 設定 GitHub Pages](?id=post%2Fpage%2Fgithubpages_zh.md "card") 行內卡片。
```
...
- `?lang=zh-hk` — UI 語言偏好，會儲存在 localStorage；內容會先嘗試匹配該語言，再按回退鏈選擇可用版本。

## 多語言
NanoSite 將網站本體語言同內容語言視為相關但獨立嘅兩層。
...
- 對每篇文章或者頁面，NanoSite 會先嘗試載入同目前 UI 語言相同嘅內容版本。若該版本不存在，就回退到 `site.yaml` 入面嘅 `defaultLanguage`；本倉庫預設係 `en`。
- 如果設定嘅預設語言版本都不存在，NanoSite 會繼續嘗試 `en`、`default`，最後使用該項目下第一個可用版本，避免頁面完全無法渲染。

內容索引支援三種形式：

- 簡化版：按語言直接提供 Markdown 路徑。
- 統一版：每種語言使用 `{title, location}`。
- 舊版：`index.en.yaml`、`index.zh.yaml` 等分語言檔案。

## SEO
NanoSite 會喺執行時按目前頁面更新 meta、Open Graph、Twitter Card 同 JSON-LD。資料來源優先順序：
